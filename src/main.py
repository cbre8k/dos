...
class Game(config.GameConfig):
    ...
    ACCUMULATE_VEL_INCREMENT = 0.001
    ACCUMULATE_POINT_INCREMENT = 0.002
    SQUARE_SIZE = 16
    ...
    #     self.background_image_index = min(self.score // 100, len(self.BACKGROUND_LEVEL) - 1)
    ...
    def unlock_random_square(self):
        locked_squares = [(i, j) for i in range(len(self.censored_grid)) for j in range(len(self.censored_grid[0])) if self.censored_grid[i][j]]
        if locked_squares:
    ...
    def change_background(self):
        if self.background_image_index < len(self.BACKGROUND_LEVEL) - 1:
            ...
            # Reinitialize the censored grid for the new background
            self.censored_grid = [
    ...
    def draw_censored_background(self):
        ...
        # Overlay censored squares
        for i, row in enumerate(self.censored_grid):
            for j, is_censored in enumerate(row):
                if is_censored:
                    pyxel.rect(self.BACKGROUND_WIDTH + j * self.SQUARE_SIZE, i * self.SQUARE_SIZE, self.SQUARE_SIZE, self.SQUARE_SIZE, pyxel.COLOR_BLACK)
